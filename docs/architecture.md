...
입력 담당은 **안드로이드 앱**(Capacitor가 `apps/web` 화면을 담은 WebView)이고, 링크를 받은 동승자는 웹 페이지(`/r`)만 엽니다. 앱 화면은 `https://localhost`에서 뜨므로 `/api` 호출은 절대 주소이고 Worker가 CORS로 허용합니다.

```mermaid
flowchart LR
  subgraph Phone[입력 담당 폰 · 안드로이드 앱]
    UI[입력·계산·로컬 저장]
  end
...
| 위치                 | 담당                                                              | 저장하는 것                                 |
|----------------------|-------------------------------------------------------------------|---------------------------------------------|
| **입력 담당 앱**     | 여행·구간·멤버 입력, 계산, 결과 링크 생성, 운전 중 잠금(속도 감지) | 여행 전체 기록, 유가 스냅샷(조회 시각 포함). 앱 안에만 |
...
| 엔드포인트                     | 입력                                                                      | 출력                                                                                                                          | 내부 동작                                                                                  |
|--------------------------------|---------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------|
| `POST /api/session`            | Turnstile 토큰 `{turnstileToken}`                                         | 서명된 세션 토큰(2시간) `{token, expiresAt}`                                                                                  | Turnstile 검증 후 HMAC 서명 토큰 발급. 나머지 /api는 `Authorization: Bearer` 토큰 없으면 즉시 거절 |
| `GET /api/places`              | q(검색어)만                                                               | `{provider, places: [{name, address, lat, lng}]}` 상위 5개                                                                    | 카카오 로컬 키워드 검색(예비 TMAP POI). 현재 위치 좌표는 받지 않음. 서버 캐시 없음, 앱 디바운스 300ms |
| `POST /api/route`              | `{points: [{lat, lng}]}` 2~11개 (하차 장소를 고르면 그 지점 추가). 이름·날짜·인원 없음 | `{provider, legs: [{distanceM, durationMin, tollWon, taxiFareWon, congestedRatio, slowRoadRatio}]}`. 2차: 지나는 고속도로 노선명과 노선별 거리 | 보통 여행 1건당 1회 호출. 하차 장소를 고른 경우에만 구간별로 추가 호출(구간마다 제공자 1건). 같은 좌표 목록은 6시간 캐시(DO) |
| `GET /api/fuel/avg`            | sido(오피넷 시·도 코드), prod(B027·D047·K015). 시·도는 폰이 주소 문자열에서 판단 | `{priceWon, updatedAt}`                                                                                                       | KV에서 읽기만 함. 외부 호출 없음. 값이 없으면 503(앱은 기본값 단가)                        |
...
| 응답 캐시      | 경로 응답(Durable Object, 6시간). 장소 검색은 캐시하지 않음 | 캐시 키는 반올림한 좌표를 비밀 값과 섞은 HMAC. 좌표 문자열이 캐시에 남지 않음 |
...
- 앱은 위치 권한을 쓰지 않는다. 서버로 가는 좌표는 사용자가 검색해서 고른 장소뿐이다.
- 서버 기반 음성 인식을 쓰지 않는다. 음성이 외부 회사 서버로 전송되기 때문이다.
- 광고를 넣지 않는다(2026-09-21 확정). 앱과 결과 보기 페이지 모두 광고 SDK·스크립트를 부르지 않는다.
- `/api`는 앱 WebView 출처와 개발 주소만 CORS로 허용한다. 허용 목록 밖 브라우저는 응답을 읽지 못한다(서버 쪽 방어는 세션 토큰과 한도가 맡는다).
...
| 앱 배포                      | 구글 플레이 개발자 등록 1회 25달러(계정 공용)                                                          | 애플(연 99달러)은 범위 밖. 서명 키는 로컬 보관                                                                                          |
...
- 앱을 처음 열 때 Turnstile(보이지 않는 모드)을 실행하고, 토큰을 `POST /api/session`으로 보낸다. 앱은 WebView라 위젯이 그대로 동작하고, 위젯 호스트에 WebView 출처를 등록한다. 앱 위조를 더 막아야 하면 Play Integrity로 바꾼다(출시 뒤 재검토).
...
- 세션 발급 자체도 IP 분당 2회로 제한해 세션 대량 생성을 막는다(Rate Limiting 바인딩은 10초·60초 주기만 지원, decisions.md M3).
...
|-----------------------|------------------------------------------------------|-------------------------------------|
| 0~70%                 | 정상                                                 | —                                   |
| 70~90% (2차)          | 세션 하루 한도 절반으로, 장소검색 최소 글자 수 2 → 3 | —                                   |
| 90~100% (2차)         | 새 카카오 호출 중단, 캐시 적중과 TMAP 예비로 응답    | —                                   |
...
- Worker 요청 한도가 소진돼도 서비스가 멈추지 않게 설계한다. 앱은 화면을 안에 들고 있고, 계산·기록·결과 링크 생성은 서버 없이 동작한다. 결과 보기 페이지도 브라우저에서만 해석한다.
...
- **비상 스위치:** KV에 `auto_route`, `auto_places` 켜기/끄기(`on`/`off`) 값, `provider`(`auto`/`kakao`/`tmap`)로 제공자 고정. 재배포 없이 대시보드에서 바로 바꾼다(Worker 메모리 캐시 때문에 최대 60초 뒤 반영).
...
| 항목 | 내용 |
|---|---|
| KV `CTRL` 키 | `auto_route`, `auto_places`, `provider`, `fuel:avg`(Cron이 씀), `block:YYYY-MM-DD` |
...
| MVP 필수                                                                                                                                                                | 2차                                                                                                              |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|
| 입력값 검증, 세션 토큰(Turnstile), 분당 제한, 일일·시간당 하드 상한, 응답 축소, 좌표 캐시, 비상 스위치, 사용률 알림, 결과 링크 형식 검증·텍스트 출력·안내 문구, 직접 입력 표시 | 단계별 절전 모드(70·90%), 공공 교통 이력 기반 운전 시간 검증(조작 의심 표시), 도메인 구매 후 WAF 이전 |
...
- **안전:** 앱은 운전이 끝난 뒤에 연다. 주행 중 입력을 전제하지 않으므로 속도 감지·운전 중 잠금을 두지 않는다.
...
- **중간 하차·합류:** 도착 후 타임라인에서 사람과 시각만 폰에 기록한다. 도착 후 타임라인에서 그 시각을 경계로 구간이 나뉘고, 장소는 검색으로 고르거나 고르지 않으면 운전 시간 비율로 거리를 나눈다. 지도 핀이나 현재 위치는 쓰지 않는다.
